...
func _process(delta: float):	
	...
	play_button.position.y = sin(10 + (timer * 1.8 * PI) / hover_time) * hover_magnitude + play_button_start_y
	credits_button.position.y = sin(15 + (timer * 1.6 * PI) / hover_time) * hover_magnitude + credits_button_start_y
	
	if (floor((timer + delta) / music_notes_move_time) > floor((timer) / music_notes_move_time)):
		for note: TextureRect in music_notes:
			note.position.y += move_up * music_notes_move_distance
		move_up *= -1
