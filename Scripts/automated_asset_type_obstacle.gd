...
func create_sprites():
	...
	var base_front_w = texture_front.get_width() * pixel_size
	var base_front_h = texture_front.get_height() * pixel_size
	front = Vector2(base_front_w, base_front_h)
	...
	front_sprite.scale.x = width / base_front_w
	front_sprite.scale.z = front_length / base_front_h
	...
	front_sprite.position = Vector3(0, 0 + offset_y, -(top_length/2))
	plat_sprite.position = Vector3(0, front_length, (top_length/2)) # opposite of front sprite
