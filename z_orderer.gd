	...
	for raycast in raycasts:
		...
		if raycast.get_collider():
			...
			if not stored_colliders.has(hit):
				stored_colliders[hit] = sprite.global_position.y
...
func change_y():
	for collider in hit_colliders:
		var sprite = collider.get_node_or_null('Sprite3D')
		sprite.global_position.y = lowered_y
	
	for item in stored_colliders.keys():
		if not hit_colliders.has(item):
			item.get_node("Sprite3D").global_position.y = stored_colliders[item]
			
		

func set_raycast_length():
	for raycast in raycasts:
		raycast.target_position.y = raycast_length
